...
function updatepanggil(transid) {
    ...
    //return;
    $.ajax({
        url: url + 'index.php/PanggilLab/updatepanggil',
        type: 'post',
        //dataType: 'json',
        //cache: false,
        data: {
...
function tampilantrian(d, i) {
    ...
    if (length > i) {
        ...
        if (nourut) {
            var utterance = new SpeechSynthesisUtterance("antrian " + nourut + "!" + depan + " " + namapasien.toLowerCase() + "!dari " + poli);
        } else {
...
function panggilantrian() {
    $.ajax({
        url: url + 'index.php/PanggilLab/panggilantrian',
        type: 'post',
        data: {
            lantai: lantai
        },
