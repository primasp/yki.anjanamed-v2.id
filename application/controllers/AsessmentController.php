...
class AsessmentController extends CI_Controller
{
    ...
    public function filterDataxxxx()
    {
        // return var_dump("123");
        // die;
        ...
        $tab = $this->input->post('tab'); // "Belum" atau "Selesai"
        $tanggal   = $this->input->post('tanggal');   // yyyy-mm-dd dari input date



        // Normalisasi tanggal: kalau kosong, pakai hari ini
        if (empty($tanggal)) {
            $tanggal = date('Y-m-d');
        }
        ...
        // =======================
        // 2. Tambah detil pemeriksaan utk poli APS
        // =======================
        if (!empty($data)) {
            foreach ($data as &$row) {                          // by reference supaya properti nempel
                if ($row->poli_id === 'APS') {
                    // method yang sama seperti di controller index
                    $row->detil_pemeriksaan = $this->am->getDetilPemeriksaanByEpisode($row->episode_id);
                } else {
                    $row->detil_pemeriksaan = [];
                }
        ...
        if (!empty($data)) {
            ...
            foreach ($data as $row) {
                ...
                // siapkan JSON untuk data-layanan (AMAN untuk dimasukkan ke atribut HTML)
                $data_layanan_attr = '[]';
                ...
                if ($tab === 'Belum') {
                    ...
                    // >>> DI SINI yang tadinya error Array to string conversion <<<
                    $aksi = "
                ...
                // $aksi = ($tab === 'Belum')
                //     ? "<button class='btn btn-sm btn-primary px-3 btnMulaiAsesmen'
                //                 data-episode='{$row->episode_id}'
                //                 data-pasien='{$row->pasien_id}'
                //                 data-poli='{$row->poli_id}'
                //                 data-kunj='{$row->kunjungan_rs}'>
                //             <i class='feather-edit-3 me-1'></i> Mulai
                //     </button>"
                //     : "<button class='btn btn-sm btn-info px-3 btnEditAsesmen'
                //             data-episode='{$row->episode_id}'
                ...
                //             data-poli='{$row->poli_id}'
                //             data-kunj='{$row->kunjungan_rs}'
                //             data-layanan='{$row->detil_pemeriksaan}'>
                //         <i class='feather-eye me-1'></i> Lihat / Edit 
                //    </button>";
                ...
                echo "
                    <tr>
                        ...
                        <td>
                            <span class='fw-semibold text-dark'>" . htmlspecialchars($row->nama_poli) . "</span>
                            {$detil}
                        </td>
